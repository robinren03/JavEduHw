...
public class ChannelActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    private void getListData(){
        String[] ss=getIntent().getStringArrayExtra(RESULT_JSON_KEY);
        System.out.println("!#!E@#!#@E@E@E@E@E@E "+ss[0].substring(0,2));
        List<ChannelBean> listAll=new ArrayList<ChannelBean>(){};
        for(int i=0;i<9;i++){
            System.out.println("i"+i+" "+ss[i]);
            String name=ss[i].substring(0,2);
            boolean sel=false;
            if(ss[i].contains("1"))sel=true;
            //System.out.println("receive:"+name+" "+sel);
            ChannelBean a1=new ChannelBean(name,sel);
            listAll.add(a1);
        }

        //List<ChannelBean> listAll = new Gson().fromJson(jsonStr,new TypeToken<List<ChannelBean>>(){}.getType());
//        ChannelBean a1=new ChannelBean("语文",true);
//        ChannelBean b1=new ChannelBean("数学",true);
//        ChannelBean c1=new ChannelBean("英语",true);
//        ChannelBean d1=new ChannelBean("物理",true);
//        ChannelBean e1=new ChannelBean("生物",true);
//        listAll.add(a1);
//        listAll.add(b1);
//        listAll.add(c1);
//        listAll.add(d1);
